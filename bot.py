...
def subscribe_command(update: Update, context: CallbackContext):
    ...
    if len(context.args) < 2:
        update.message.reply_text("Usage: /subscribe PAIR INTERVAL_SECONDS")
        return
    pair = context.args[0].upper()
    interval = int(context.args[1])
    if pair not in SUPPORTED_PAIRS:
        update.message.reply_text(f"Unsupported pair. Supported: {SUPPORTED_PAIRS}")
        return
    if chat_id not in subscriptions:
        subscriptions[chat_id] = {}
    subscriptions[chat_id][pair] = interval
    threading.Thread(target=alert_pair, args=(chat_id, pair, interval), daemon=True).start()
    update.message.reply_text(f"Subscribed to {pair} with interval {interval}s!")

def unsubscribe_command(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    if len(context.args) < 1:
        update.message.reply_text("Usage: /unsubscribe PAIR")
        return
    pair = context.args[0].upper()
    if chat_id in subscriptions and pair in subscriptions[chat_id]:
        subscriptions[chat_id][pair] = 0
        update.message.reply_text(f"Unsubscribed from {pair}")
    else:
        update.message.reply_text(f"You were not subscribed to {pair}")
...
def status_command(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    if chat_id not in subscriptions or not subscriptions[chat_id]:
        update.message.reply_text("No active subscriptions. Use /subscribe PAIR INTERVAL")
        return
    messages = []
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if data in SUPPORTED_PAIRS:
        ...
        if img:
            with open(img, "rb") as photo:
                query.message.reply_photo(photo=photo, caption=desc)
        else:
            query.message.reply_text(desc)
    elif data == "refresh":
        query.message.reply_text("Prices refreshed. Click a pair button to get latest signal.")
